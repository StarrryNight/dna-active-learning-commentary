import torch
import torch.nn as nn
from torch.utils.data import DataLoader
from pathlib import Path
import numpy as np
from typing import Optional, Any
from tqdm import tqdm
import json

#Class trainer to train the model
class Trainer:
    #Takes in parameters like data or model
    def __init__(
        self,
        model: nn.Module,
        train_dataloader: DataLoader,
        val_dataloader: Optional[DataLoader],
        model_dir: str | Path,
        num_epochs: int = 80,
        lr: float = 5e-3,
        weight_decay: float = 0.01,
        device: torch.device = torch.device("cpu")
    ):
        #Set up the properties of this trainer, such as data model optimizer scheduler etc
        self.device = device
        self.model = model.to(device)
        self.train_dataloader = train_dataloader
        self.val_dataloader = val_dataloader
        self.model_dir = Path(model_dir)
        self.model_dir.mkdir(parents=True, exist_ok=True)
        self.num_epochs = num_epochs

        div_factor = 25.0
        min_lr = lr / div_factor

        self.optimizer = torch.optim.AdamW(
            self.model.parameters(),
            lr=min_lr,
            weight_decay=weight_decay
        )
        self.scheduler = torch.optim.lr_scheduler.OneCycleLR(
            self.optimizer,
            max_lr=lr,
            div_factor=div_factor,
            steps_per_epoch=len(train_dataloader),
            epochs=num_epochs,
            pct_start=0.3,
            three_phase=False
        )
        self.regression_criterion = nn.MSELoss()
        self.classification_criterion = nn.KLDivLoss(reduction= "batchmean")
        self.best_pearson = -float("inf")

    #Set up each train step
    def train_step(self, batch: dict[str, Any]) -> float:
        # put the batches into gpu
        self.model.train()
        x = batch["x"].to(self.device)
        y = batch["y"].to(self.device).float()

        #Use the model to calculate y_pred
        y_pred = self.model(x).squeeze()
        if "y_probs" in batch: # yeast, classification
            y_probs = batch["y_probs"].to(self.device)
            loss = self.classification_criterion(y_pred, y_probs)
        else: # regression
            y = batch["y"].to(self.device)
            loss = self.regression_criterion(y_pred, y.squeeze(-1))

        loss.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
        self.scheduler.step()
        #Process the steps
        return loss.item()

    def validate(self) -> dict[str, float]:
        self.model.eval()
        y_true, y_pred = [], []

        #Validate using batch
        with torch.no_grad():
            for batch in tqdm(self.val_dataloader, desc="Validation"):
                x = batch["x"].to(self.device)
                y = batch["y"].to(self.device).float()
                pred = self.model.predict(x).squeeze()

                y_true.append(y.cpu().numpy())
                y_pred.append(pred.cpu().numpy())

        #Now we have two lists
        y_true = np.concatenate(y_true)
        y_pred = np.concatenate(y_pred)

        #Show the metrics
        metrics = {
            "MSE": float(np.mean((y_pred - y_true) ** 2)),
            "pearsonr": float(np.corrcoef(y_true, y_pred)[0, 1])
        }
        return metrics

    #Obviously save the model
    def save_model(self, epoch: str):
        torch.save(self.model.state_dict(), self.model_dir / f"model_{epoch}.pth")
        torch.save(self.optimizer.state_dict(), self.model_dir / f"optimizer_{epoch}.pth")
        torch.save(self.scheduler.state_dict(), self.model_dir / f"scheduler_{epoch}.pth")

    #Function that starts all the training 
    def fit(self):
        #Train the epochs
        for epoch in tqdm(range(1, self.num_epochs + 1)):
            epoch_losses = []
            for batch in tqdm(self.train_dataloader, desc=f"Epoch {epoch}"):
                loss = self.train_step(batch)
                epoch_losses.append(loss)

            #Open loss.json file
            with open(self.model_dir / "losses.json", "a") as f:
                json.dump({f"epoch_{epoch}": epoch_losses}, f)
                f.write("\n")

            #Validate after training in each epoch
            if self.val_dataloader is not None:
                metrics = self.validate()
                with open(self.model_dir / "val_metrics.json", "a") as f:
                    json.dump({f"epoch_{epoch}": metrics}, f)
                    f.write("\n")
                print(metrics)

            #If pearson higher than current highest model, save it 
                if metrics["pearsonr"] > self.best_pearson:
                    self.best_pearson = metrics["pearsonr"]
                    self.save_model("best")
            # self.save_model(f"{epoch}")
